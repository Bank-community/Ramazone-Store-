...
// Default to ALL_AREAS if not found
...
function loadData() {
    try {
        ...
        // Get User Location from LocalStorage (Set by Home Page)
        ...
        if (cached) {
            ...
            allProducts = rawProducts.filter(p => {
                // 1. Remove Disabled/Hidden Products
                if (p.isVisible === false) return false;

                // 2. Remove Products Not Available in User's Location
                // If availableAreas is empty/undefined, assume available everywhere
                // If it has values, userLoc MUST be in the list
                if (p.availableAreas && Array.isArray(p.availableAreas) && p.availableAreas.length > 0) {
                    if (!p.availableAreas.includes(userLoc)) {
                        return false; // Hide this product
                    }
                }
                return true; // Show product
            });
            
            // Render Categories
            renderCategoryChips(data.homepage?.normalCategories || []);
            
            // Render Initial Trending Products
...
function renderSearchHistory() {
    ...
    history.slice(0, 3).forEach(term => { 
        ...
        <div class="history-item" onclick="executeSearch('${term}')">
            ...
            <i class="fas fa-arrow-left history-arrow" style="transform: rotate(45deg);"></i>
        </div>`;
    });
...
function renderCategoryChips(categories) {
    ...
    // Merge categories from config and filtered products
    const uniqueCats = new Set();
    
    // First add from homepage config (if valid)
    categories.forEach(c => uniqueCats.add(c.name));

    // Then add from actual available products
    allProducts.forEach(p => { if(p.category) uniqueCats.add(p.category); });

    let html = '';
    uniqueCats.forEach(cat => {
        html += `<span class="cat-chip" onclick="executeSearch('${cat}')">${cat}</span>`;
    });
    container.innerHTML = html;
}
...
function renderTrendingProducts() {
    ...
    if(allProducts.length === 0) {
        container.innerHTML = '<p class="p-4 text-xs text-gray-400">No products available in your area.</p>';
        return;
    }
    // Pick 6 random products from FILTERED list
    const trending = allProducts.sort(() => 0.5 - Math.random()).slice(0, 6);
    
    let html = "";
    trending.forEach(p => {
        html += createListCard(p); 
    });
    container.innerHTML = html;
}
...
// ... (Card Generators - Same as before) ...
function createListCard(p) {
    ...
    return `
    <div class="prod-card-list" onclick="location.href='product-details.html?id=${p.id}'">
        <div class="list-img-box">
            <img src="${p.images?.[0] || 'placeholder.jpg'}" alt="${p.name}">
        </div>
        <div class="list-info-box">
            <h3 class="list-title">${p.name}</h3>
            <div class="flex items-center gap-2 mt-1">
                 <span class="list-price">${price}</span>
                 <span class="list-old-price">${oldPrice}</span>
                 <span class="text-xs text-green-600 font-bold">${discount}</span>
            </div>
        </div>
    </div>`;
}
...
function createGridCard(p) {
    ...
    return `
    <div class="prod-card-grid" onclick="location.href='product-details.html?id=${p.id}'">
        <div class="grid-img-box">
            <img src="${p.images?.[0] || 'placeholder.jpg'}" alt="${p.name}">
        </div>
        <div class="flex flex-col">
            <h3 class="grid-title">${p.name}</h3>
            <div class="mt-1">${rating}</div>
            <div class="grid-price">${price} <span class="text-xs text-gray-400 font-normal line-through ml-1">${oldPrice}</span></div>
        </div>
    </div>`;
}