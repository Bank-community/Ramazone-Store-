// product-search.js - Header Cart, Compact Layout, Firebase Fallback, Limit History to 3, Wishlist Logic
...
const WISHLIST_KEY = "ramazoneWishlist"; // New Key
...
function toggleWishlist(id, btnElement) {
    ...
    if (index > -1) {
        ...
        list.splice(index, 1);
        if(btnElement) {
            btnElement.className = "far fa-heart wishlist-icon";
            btnElement.style.color = "#c2c2c2";
        }
    } else {
        // Add
        list.push(id);
        if(btnElement) {
            btnElement.className = "fas fa-heart wishlist-icon";
            btnElement.style.color = "#ef4444"; // Red color
        }
    }
    saveWishlist(list);
}
...
function fetchFromFirebase() {
    ...
    database.ref('ramazone/products').once('value')
        ...
        .catch((error) => {
            ...
            if(loader) loader.innerHTML = '<p class="text-red-500">Failed to load data. Please refresh.</p>';
        });
}
...
function renderCategoryChips() {
    ...
    categories.forEach(cat => {
        ...
        html += `<div onclick="executeSearch('${cat}')" class="px-3 py-1.5 rounded-full text-xs font-semibold cursor-pointer border border-transparent hover:border-gray-200 transition-colors ${colorClass}">${cat}</div>`;
        i++;
    });
...
function loadMore() {
    ...
    let html = "";
    batch.forEach(p => { html += createResultCard(p); });
...
function getStarRatingHTML(rating) {
    ...
    let stars = '';
    for (let i = 1; i <= 5; i++) {
        if (rating >= i) stars += '<i class="fas fa-star star-filled"></i>';
        else if (rating >= i - 0.5) stars += '<i class="fas fa-star-half-alt star-filled"></i>';
        else stars += '<i class="fas fa-star star-empty"></i>';
    }
    return `<div class="star-rating-row">${stars}</div>`;
}

// --- FLIPKART STYLE CARD RENDERER (Updated with Wishlist) ---
function createResultCard(p) {
    ...
    // Wishlist Logic for Icon
    const wishlist = getWishlist();
    const isWishlisted = wishlist.includes(p.id);
    const heartClass = isWishlisted ? "fas fa-heart" : "far fa-heart";
    const heartColor = isWishlisted ? "color: #ef4444;" : "color: #c2c2c2;";

    let variantCount = 0;
    if (p.groupId) variantCount = allProducts.filter(prod => prod.groupId === p.groupId).length;

    let variantTag = '';
    if (variantCount > 1) {
        variantTag = `<div class="variant-tag" onclick="event.stopPropagation(); openVariantModal('${p.id}', '${p.groupId}')">${variantCount} Variants</div>`;
    } else if (p.variants && p.variants.length > 1) {
        variantTag = `<div class="variant-tag" onclick="event.stopPropagation(); openVariantModal('${p.id}', null)">${p.variants.length} Variants</div>`;
    }

    const isGrocery = isGroceryCategory(p.category || "");
    const addToCartBtn = isGrocery ? `<button class="add-cart-btn" onclick="event.stopPropagation(); addToCartSimple('${p.id}', ${p.displayPrice})">Add to Cart</button>` : '';

    return `
    <div class="prod-card" onclick="location.href='product-details.html?id=${p.id}'">
        <div class="img-container">
            <i class="${heartClass} wishlist-icon" style="${heartColor} font-size: 1.2rem;" onclick="event.stopPropagation(); toggleWishlist('${p.id}', this)"></i>
...
function addToCartSimple(productId, price) {
    try {
        ...
        else cart.push({ id: productId, quantity: 1, variants: {} });
        ...
        setTimeout(() => { btn.innerText = originalText; btn.style.backgroundColor = "white"; btn.style.color = "#2874f0"; }, 1500);
...
function updateHeaderCart() {
    ...
    let cart = JSON.parse(localStorage.getItem('ramazoneCart')) || [];
    if (cart.length === 0) { cartBtn.style.display = 'none'; return; }
    ...
    cart.forEach(item => {
        ...
        if (product) totalPrice += (Number(product.displayPrice) * item.quantity);
    });
...
function openVariantModal(currentProductId, groupId) {
    ...
    if (groupId) variants = allProducts.filter(p => p.groupId === groupId);
    else {
    ...
    let variantsHtml = variants.map(v => {
        ...
        <div class="variant-item ${isSelected ? 'selected' : ''}" onclick="location.href='product-details.html?id=${v.id}'">
            ...
            <div class="v-info">
                ...
                <div><span class="v-price">₹${Number(v.displayPrice).toLocaleString()}</span></div>
            </div>
...
function renderSearchHistory() {
    ...
    history.slice(0, 3).forEach(term => { 
        html += `<div class="flex items-center p-3 border-b border-gray-50 cursor-pointer hover:bg-gray-50" onclick="executeSearch('${term}')"><i class="fas fa-clock text-gray-400 mr-3"></i><span class="flex-grow text-sm text-gray-700">${term}</span><i class="fas fa-arrow-left -rotate-45 text-gray-300"></i></div>`;
    });
    container.innerHTML = html;
}
