// --- GLOBAL STATE (SUPERSONIC CACHE V2) ---
...
const INITIAL_DEALS_LOAD = 10; 
const DEALS_BATCH_SIZE = 6;    
...
const CACHE_KEY_HTML = "RAMAZONE_HTML_CACHE_V2";
...
function setupInstallButton() {
    ...
    if (installBtn) {
        installBtn.addEventListener('click', async () => {
            ...
            if (!deferredInstallPrompt) { showToast('Installation is not available right now.', 'error'); return; }
...
// --- CART FUNCTIONS (RESTORED BADGE LOGIC) ---
...
// FIX: Added Badge Logic Back
function updateCartIcon() { 
    const totalQuantity = getTotalCartQuantity(); 
    const cartCountElement = document.getElementById('cart-item-count'); 
    if (cartCountElement) { 
        cartCountElement.textContent = totalQuantity > 0 ? totalQuantity : ''; 
        if(totalQuantity > 0) cartCountElement.classList.remove('hidden');
...
// --- SUPERSONIC INITIALIZATION ---
document.addEventListener('DOMContentLoaded', () => {
    loadFromCache();
    initializeApp();
    updateCartIcon(); // Init badge on load
});
...
// --- SMART HTML LOADER (UPDATED FOR SLIDER MODULARITY) ---
async function loadPageStructure() {
    const mainArea = document.getElementById('main-content-area');
    if (mainArea.childElementCount > 0) return; 
    
    // UPDATED: Added slider.html at the TOP of the list
    const sections = [
        'slider.html', 
        'location-popup.html', 'categories.html', 'recently-viewed.html', 
        'festive-collection.html', 'flip-card.html', 
        'just-for-you.html', 'single-banner-section.html', 'deals-of-the-day.html'
    ];

    const cachedHTML = JSON.parse(localStorage.getItem(CACHE_KEY_HTML) || '{}');
    let htmlContent = '';
    let missingSections = [];

    sections.forEach(s => {
        if (cachedHTML[s]) { htmlContent += cachedHTML[s]; } 
        else { missingSections.push(s); }
    });

    if (htmlContent) mainArea.innerHTML = htmlContent;
    ...
    try {
        ...
        localStorage.setItem(CACHE_KEY_HTML, JSON.stringify(newCache));
        
        if (!htmlContent) mainArea.innerHTML = fullHtml;
        
        const searchContainer = document.getElementById('search-bar-container');
        if(searchContainer && !searchContainer.innerHTML) {
             const resp = await fetch('sections/search-bar.html');
             searchContainer.innerHTML = await resp.text();
        }
...
function processAndRenderData(data, isCached) {
    ...
    if (!isCached) checkAndShowLocationWelcomePopup(); 
...
function renderAllSections(data) {
    ...
    // No Videos, No Marquee
    
    const festiveData = homepageData.festiveCollection;
    if (festiveData && festiveData.productIds) {
         const limitedFestive = { ...festiveData, productsToShow: 5 };
         renderFestiveCollection(limitedFestive); 
    }
    ...
    setupSideMenu();
    setupInstallButton();
    updateCartIcon(); // Update Icon Count
    setupScrollAnimations();
    setupHeaderScrollEffect();
    setupHomepageSearch(); 
}

// --- DEALS INFINITE SCROLL (SAME) ---
...
function renderHighlightedProducts() {
    ...
    loadMoreDeals(INITIAL_DEALS_LOAD); 
    ...
    if (loader) {
        ...
        dealsObserver = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting && !isLoadingDeals) { 
                loadMoreDeals(DEALS_BATCH_SIZE); 
            }
        }, { threshold: 0.1 }); 
...
function loadMoreDeals(count) {
    const wrapper = document.getElementById('highlighted-products-wrapper');
    const loader = document.getElementById('deals-loader');
    if (isLoadingDeals || !wrapper) return;
    ...
    const productsToLoad = dealsOfTheDayProducts.slice(currentlyDisplayedDeals, currentlyDisplayedDeals + count);
...
function renderFestiveCollection(collectionData) {
    ...
    const limit = Math.min(collectionData.productsToShow || 5, 5); 
    let productsHTML = '';
    let productsFound = 0;
    for (let i = 0; i < collectionData.productIds.length; i++) {
        if (productsFound >= limit) break;
        const id = collectionData.productIds[i];
...
function setupLocationSelectionsFromStorage() {
    ...
    if (!savedLoc) { localStorage.setItem('userLocation', DEFAULT_LOCATION_KEY); currentSelectedState = null; currentSelectedDistrict = null; return; }
    if (savedLoc === DEFAULT_LOCATION_KEY) { currentSelectedState = null; currentSelectedDistrict = null; } 
    else { const parts = savedLoc.split('/'); if (parts.length >= 2) { currentSelectedState = parts[0]; currentSelectedDistrict = parts[1]; } else { localStorage.setItem('userLocation', DEFAULT_LOCATION_KEY); currentSelectedState = null; currentSelectedDistrict = null; } }
}
function filterProductsByLocation() {
    const currentLoc = localStorage.getItem('userLocation');
    if (currentLoc === DEFAULT_LOCATION_KEY) { filteredProductsCache = allProductsCache.filter(product => !product.availableAreas || !Array.isArray(product.availableAreas) || product.availableAreas.length === 0); } 
    else { filteredProductsCache = allProductsCache.filter(product => { const isAllArea = !product.availableAreas || !Array.isArray(product.availableAreas) || product.availableAreas.length === 0; const isAtLocation = product.availableAreas && product.availableAreas.includes(currentLoc); return isAllArea || isAtLocation; }); }
}
function rerenderProductSections() {
    const fullData = window.ramazoneData || { homepage: {} };
    renderRecentlyViewed();
    const limitedFestive = { ...fullData.homepage.festiveCollection, productsToShow: 5 };
    renderFestiveCollection(limitedFestive);
...
function setupLocationSystem() {
    ...
    if (areaSearchInput) { areaSearchInput.addEventListener('input', (e) => { const query = e.target.value.toLowerCase(); if (currentSelectedState && currentSelectedDistrict) { renderAreaList(currentSelectedState, currentSelectedDistrict, query, allLocationsCache, localStorage.getItem('userLocation')); } }); }
}
function openLocationPopup() {
    const overlay = document.getElementById('location-overlay');
    const panel = document.getElementById('location-selector-panel');
    const body = document.body;
    if (overlay && panel) { setupLocationSelectionsFromStorage(); renderStateTabs(allLocationsCache, currentSelectedState); if (currentSelectedState) { renderDistrictTabs(currentSelectedState, allLocationsCache, currentSelectedDistrict); } if (currentSelectedState && currentSelectedDistrict) { renderAreaList(currentSelectedState, currentSelectedDistrict, '', allLocationsCache, localStorage.getItem('userLocation')); } overlay.classList.add('visible'); panel.classList.add('open'); body.classList.add('location-open'); }
}
function closeLocationPopup() {
    const overlay = document.getElementById('location-overlay');
    const panel = document.getElementById('location-selector-panel');
    const body = document.body;
    if (overlay && panel) { overlay.classList.remove('visible'); panel.classList.remove('open'); body.classList.remove('location-open'); }
}
function setupHomepageSearch() {
    ...
    if(allCategoriesCache.length) { categorySuggestionsContainer.innerHTML = allCategoriesCache.map(cat => `<span class="suggestion-tag" data-category="${cat.name}">${cat.name}</span>`).join(''); categorySuggestionsContainer.addEventListener('click', e => { if (e.target.classList.contains('suggestion-tag')) { window.location.href = `./products.html?category=${encodeURIComponent(e.target.dataset.category)}`; } }); }
    searchInput.addEventListener('input', () => {
        const query = searchInput.value.toLowerCase();
        if (query.length < 1) { suggestionsContainer.classList.add('hidden'); return; }
        const suggestions = filteredProductsCache.filter(p => { const nameMatch = p.name.toLowerCase().includes(query); let keywordMatch = false; if (p.product_of_keyword && Array.isArray(p.product_of_keyword)) { keywordMatch = p.product_of_keyword.some(k => k.toLowerCase().startsWith(query)); } return nameMatch || keywordMatch; }).slice(0, 5);
        if (suggestions.length > 0) { suggestionsContainer.innerHTML = suggestions.map(prod => `<a href="./product-details.html?id=${prod.id}" class="suggestion-item"><img src="${(prod.images && prod.images[0]) || 'https://placehold.co/100x100/e2e8f0/64748b?text=?'}" alt="${prod.name}"><span class="text-sm text-gray-700">${prod.name}</span></a>`).join(''); suggestionsContainer.classList.remove('hidden'); } else { suggestionsContainer.classList.add('hidden'); }
    });
    ...
    if (headerSearchTrigger) { headerSearchTrigger.addEventListener('click', () => { searchInput.scrollIntoView({ behavior: 'smooth', block: 'center' }); setTimeout(() => searchInput.focus(), 300); }); }
}
function renderSearch(searchData) { 
    const searchInput = document.getElementById('home-search-input'); 
    if (!searchInput || !searchData?.scrollingTexts?.length) { if(searchInput) searchInput.placeholder = "Search for products..."; return; } 
    const texts = searchData.scrollingTexts; let i = 0; if (window.searchInterval) clearInterval(window.searchInterval); 
    const updatePlaceholder = () => { if (searchInput && document.activeElement !== searchInput) { searchInput.placeholder = `Search for ${texts[i]}...`; i = (i + 1) % texts.length; } }; 
    updatePlaceholder(); window.searchInterval = setInterval(updatePlaceholder, 3000); 
}
...
function setupGlobalEventListeners() {
    ...
    document.body.addEventListener('click', function (event) {
        ...
        if (addButton) { event.preventDefault(); const productId = addButton.dataset.id; if (productId) { addToCart(productId); if (addButton.classList.contains('cart-btn')) { const cartIcon = addButton.querySelector('svg'); const checkIcon = addButton.querySelector('.cart-added-icon'); addButton.classList.add('added'); if (cartIcon) cartIcon.style.display = 'none'; if (checkIcon) checkIcon.style.display = 'inline-block'; setTimeout(() => { addButton.classList.remove('added'); if (cartIcon) cartIcon.style.display = 'block'; if (checkIcon) checkIcon.style.display = 'none'; }, 1500); } } return; }
        ...
        if (stateTab && !stateTab.dataset.district) { currentSelectedState = stateTab.dataset.state; currentSelectedDistrict = null; document.getElementById('loc-area-search-input').value = ''; renderStateTabs(allLocationsCache, currentSelectedState); renderDistrictTabs(currentSelectedState, allLocationsCache, null); return; }
        const districtTab = event.target.closest('.loc-tab-btn[data-district]');
        if (districtTab) { currentSelectedDistrict = districtTab.dataset.district; document.getElementById('loc-area-search-input').value = ''; renderDistrictTabs(currentSelectedState, allLocationsCache, currentSelectedDistrict); renderAreaList(currentSelectedState, currentSelectedDistrict, '', allLocationsCache, localStorage.getItem('userLocation')); return; }
        const locItem = event.target.closest('.location-item');
        if (locItem) { const selectedLocPath = locItem.dataset.path; const currentLoc = localStorage.getItem('userLocation'); if (selectedLocPath === currentLoc) { closeLocationPopup(); return; } localStorage.setItem('userLocation', selectedLocPath); setupLocationSelectionsFromStorage(); const headerText = document.getElementById('header-location-text'); if (headerText) headerText.textContent = formatLocation(selectedLocPath); renderAreaList(currentSelectedState, currentSelectedDistrict, '', allLocationsCache, selectedLocPath); filterProductsByLocation(); rerenderProductSections(); setTimeout(closeLocationPopup, 200); return; }
    });
}
