...
async function checkAutoOrderStatus() {
    ...
    // UPDATED: Search in rejected path as well
    ...
    renderDeliveryTracker(orderData.status, trackerContainer); // This will now handle 'Rejected' status
...
function setupEventListeners() {
    ...
    document.getElementById('order-items-container').addEventListener('click', function(event) { const target = event.target.closest('button'); if (!target) return; const productId = target.dataset.id; const itemIndex = orderItems.findIndex(item => item.id == productId); if (itemIndex === -1) return; if (target.classList.contains('qty-increase')) orderItems[itemIndex].quantity++; else if (target.classList.contains('qty-decrease')) { if (orderItems[itemIndex].quantity > 1) orderItems[itemIndex].quantity--; else orderItems.splice(itemIndex, 1); } saveCart(orderItems); if(orderItems.length === 0) loadOrderFromCart(); else { renderOrderItems(); updatePriceSummary(); } });
...
async function placeOrder(event) {
    ...
    if (!form.checkValidity()) { form.reportValidity(); showToast('Please fill shipping details.', 'error'); return; }
    ...
    const orderData = { orderId, customerDetails, grandTotal, paymentMethod: document.querySelector('input[name="payment"]:checked').value, deliveryMethod: document.querySelector('input[name="delivery"]:checked').value, items: orderItems.map(item => ({ id: item.id, name: item.name, quantity: item.quantity, displayPrice: item.displayPrice, image: item.images?.[0] || '' })), priceSummary: { subtotal, coupon: appliedCoupon, deliveryFee, grandTotal }, status: 'Pending', createdAt: firebase.database.ServerValue.TIMESTAMP, statusHistory: { initial: { status: 'Pending', timestamp: firebase.database.ServerValue.TIMESTAMP } } };
    try {
        ...
        // FIXED: WhatsApp Message with full price details
        ...
        saveCart([]);
        document.getElementById('order-page-content').innerHTML = `<div class="text-center p-8 bg-white rounded-lg shadow"><i class="fas fa-check-circle text-5xl text-green-500 mb-4"></i><h2 class="text-2xl font-bold text-gray-800">Order Placed Successfully!</h2><p class="mt-4 font-semibold text-lg">Your Order ID is:</p><div class="bg-gray-100 text-gray-800 font-bold text-2xl p-3 rounded-lg mt-2 inline-block select-all">${orderId}</div><p class="text-sm text-gray-500 mt-2">We will keep you updated on this page.</p><a href="index.html" class="shop-now-btn mt-6">Continue Shopping</a></div>`;
...
function renderDeliveryTracker(status, container) {
    ...
    let stepsHtml = statuses.map((s, index) => { const isCompleted = index <= currentStatusIndex; return `<div class="tracker-step ${isCompleted ? 'completed' : ''}"><div class="step-icon"><i class="fas ${icons[index]}"></i></div><p class="step-label">${s.replace(' ', '\n')}</p></div>`; }).join('');
...
async function viewInvoice() {
    ...
    slipContent.innerHTML = `
        <div style="font-family: 'Inter', sans-serif; color: #333;">
            <header style="display: flex; justify-content: space-between; align-items: flex-start; padding-bottom: 1rem; border-bottom: 2px solid #eee;">
                <div><img src="https://i.ibb.co/2RySQ5K/20240813-084352.png" alt="Ramazone Logo" style="height: 40px;"><p style="font-size: 0.8rem; color: #777; margin-top: 4px;">Ramazone Store</p></div>
                <div style="text-align: right;"><h2 style="font-size: 1.8rem; font-weight: bold; margin: 0; color: #000;">INVOICE</h2><p style="margin: 4px 0 0;"><strong>Invoice #:</strong> ${orderData.orderId}</p><p style="margin: 4px 0 0;"><strong>Date:</strong> ${new Date(orderData.createdAt).toLocaleDateString()}</p></div>
            </header>
            <section style="margin-top: 2rem; display: flex; justify-content: space-between;">
                <div><p style="font-weight: bold; color: #555;">BILLED TO:</p><p style="margin: 4px 0 0;">${orderData.customerDetails.name}</p><p style="margin: 4px 0 0; font-size: 0.9rem; color: #666; max-width: 250px;">${orderData.customerDetails.address}</p></div>
            </section>
            <section style="margin-top: 2rem;">
                <table style="width: 100%; border-collapse: collapse; font-size: 0.9rem;">
                    <thead style="background-color: #f9fafb;"><tr><th style="padding: 0.75rem; text-align: left; border-bottom: 1px solid #eee;">#</th><th style="padding: 0.75rem; text-align: left; border-bottom: 1px solid #eee;">ITEM</th><th style="padding: 0.75rem; text-align: center; border-bottom: 1px solid #eee;">QTY</th><th style="padding: 0.75rem; text-align: right; border-bottom: 1px solid #eee;">PRICE</th><th style="padding: 0.75rem; text-align: right; border-bottom: 1px solid #eee;">TOTAL</th></tr></thead>
                    <tbody>${orderData.items.map((item, index) => `<tr style="border-bottom: 1px solid #eee;"><td style="padding: 0.75rem;">${index + 1}</td><td style="padding: 0.75rem;"><div style="display: flex; align-items: center;"><img src="${item.image}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px; margin-right: 10px;" crossOrigin="anonymous"><span>${item.name}</span></div></td><td style="padding: 0.75rem; text-align: center;">${item.quantity}</td><td style="padding: 0.75rem; text-align: right;">₹${item.displayPrice.toLocaleString('en-IN')}</td><td style="padding: 0.75rem; text-align: right;">₹${(item.displayPrice * item.quantity).toLocaleString('en-IN')}</td></tr>`).join('')}</tbody>
                </table>
            </section>
            <section style="margin-top: 2rem; display: flex; justify-content: flex-end;">
                <div style="width: 280px; font-size: 0.9rem;">
                    ...
                    <div style="display: flex; justify-content: space-between; padding: 0.75rem 0; margin-top: 0.5rem; border-top: 2px solid #000; font-weight: bold; font-size: 1.2rem;"><span>Grand Total:</span><span>₹${summary.grandTotal.toLocaleString('en-IN')}</span></div>
                </div>
            </section>
            <footer style="text-align: center; font-size: 0.8rem; color: #888; margin-top: 4rem; border-top: 1px solid #eee; padding-top: 1rem;"><p>Thank you for shopping with Ramazone!</p></footer>
        </div>`;
