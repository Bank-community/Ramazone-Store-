...
// --- NEW LOGIC TO PROCESS CART ITEMS INTO DISPLAYABLE ORDER ITEMS ---
function processCartForDisplay() {
    const cart = getCart();
    const processedItems = [];
    const bundledIds = new Set(); // To keep track of items already part of a bundle

    // First pass: Identify and group bundles
    for (let i = 0; i < cart.length; i++) {
        if (bundledIds.has(cart[i].id)) continue;

        const product1 = allProductsCache.find(p => p.id === cart[i].id);
        ...
        // Find the linked product in the rest of the cart
        for (let j = i + 1; j < cart.length; j++) {
            if (cart[j].id === linkedProductId) {
                const product2 = allProductsCache.find(p => p.id === linkedProductId);
                if (product2) {
                    processedItems.push({
                        isBundle: true,
                        bundlePrice: bundleInfo.bundlePrice,
                        quantity: cart[i].quantity, // Assuming bundle quantities are the same
                        products: [
                            { ...product1, ...cart[i] },
                            { ...product2, ...cart[j] }
                        ]
                    });
                    bundledIds.add(product1.id);
                    bundledIds.add(product2.id);
                    break; // Found the pair, move to the next item in the outer loop
                }
            }
        }
    }

    // Second pass: Add all other non-bundled items
    cart.forEach(cartItem => {
        if (!bundledIds.has(cartItem.id)) {
...
function loadOrderFromCart() {
    ...
    orderItems = processCartForDisplay(); // Use the new processing function

    if (orderItems.length === 0 && getCart().length > 0) {
        // This case handles if product data is not found for cart items
...
// --- UPDATED RENDER FUNCTION TO HANDLE PACKS AND BUNDLES ---
...
// --- NEW HELPER TO CREATE A SINGLE/PACKED ITEM CARD ---
        ...
        <div class="flex-grow flex flex-col justify-between self-stretch">
            ...
            <div class="flex items-center justify-between mt-2">
                ...
                <div class="quantity-selector-order">
                    <button class="qty-decrease" data-id="${item.id}" ${isPack ? 'disabled' : ''}>-</button>
                    <span>${item.quantity}</span>
                    <button class="qty-increase" data-id="${item.id}" ${isPack ? 'disabled' : ''}>+</button>
                </div>
            </div>
        </div>
    </div>`;
...
// --- NEW HELPER TO CREATE A BUNDLE CARD ---
function createBundleItemCard(bundle) {
    ...
    return `<div class="order-item-card bg-indigo-50 p-3 rounded-lg border border-indigo-200">
        ...
        <div class="flex justify-end mt-2">
             <div class="quantity-selector-order">
                <button class="qty-decrease" data-bundle-id1="${p1.id}" data-bundle-id2="${p2.id}">-</button>
                <span>${bundle.quantity}</span>
                <button class="qty-increase" data-bundle-id1="${p1.id}" data-bundle-id2="${p2.id}">+</button>
            </div>
        </div>
    </div>`;
}
...
// --- UPDATED PRICE SUMMARY TO HANDLE PACKS AND BUNDLES ---
function updatePriceSummary() {
    const subtotal = orderItems.reduce((acc, item) => {
        if (item.isBundle) {
            return acc + (Number(item.bundlePrice) * item.quantity);
        }
        if (item.pack && item.pack.name !== 'Single Item') {
            return acc + (Number(item.pack.price) * item.quantity);
        }
...
// --- UPDATED EVENT LISTENERS & HANDLERS ---
function setupEventListeners() {
    ...
    // UPDATED: Centralized quantity change handler
    document.getElementById('order-items-container').addEventListener('click', handleQuantityChange);
}
...
function handleQuantityChange(event) {
    ...
    const isDecrease = target.classList.contains('qty-decrease');
    if (!isIncrease && !isDecrease) return;

    const cart = getCart();
    let cartUpdated = false;

    // Handle bundle quantity
    const bundleId1 = target.dataset.bundleId1;
    if (bundleId1) {
        const bundleId2 = target.dataset.bundleId2;
        const item1Index = cart.findIndex(i => i.id === bundleId1);
        const item2Index = cart.findIndex(i => i.id === bundleId2);

        if (item1Index > -1 && item2Index > -1) {
            let currentQty = cart[item1Index].quantity;
            if (isIncrease) {
                currentQty++;
            } else {
                currentQty--;
            ...
            if (currentQty > 0) {
                cart[item1Index].quantity = currentQty;
                cart[item2Index].quantity = currentQty;
            } else {
                // Remove both items if quantity drops to 0
                cart.splice(Math.max(item1Index, item2Index), 1);
                cart.splice(Math.min(item1Index, item2Index), 1);
            }
            cartUpdated = true;
        }
    } else { // Handle single item quantity
        const productId = target.dataset.id;
        const itemIndex = cart.findIndex(item => item.id === productId && !item.pack); // Ensure it's not a pack
        if (itemIndex > -1) {
            if (isIncrease) {
...
// --- UNCHANGED FUNCTIONS (placeOrder, searchOrder, invoice, etc.) ---
// NOTE: I've slightly updated `placeOrder` to correctly format the WhatsApp message for packs/bundles.
async function placeOrder(event) {
    ...
    // Create a flat list of items for the database
    const dbItems = [];
    orderItems.forEach(item => {
        if (item.isBundle) {
            item.products.forEach(p => {
                dbItems.push({ id: p.id, name: p.name, quantity: item.quantity, displayPrice: p.displayPrice, image: p.images?.[0] || '', variants: p.variants, bundleParent: item.products[0].id });
            });
        } else {
