...
async function fetchAllData() {
    ...
    if (snapshot.exists()) {
        ...
        allProductsCache = Object.values(productsObject);
...
// UPDATED: To handle variants from cart
function loadOrderFromCart() {
    ...
    orderItems = cart.map(cartItem => {
        ...
        // Combine product details with cart item info (quantity, variants)
        return { ...productDetails, quantity: cartItem.quantity, selectedVariants: cartItem.variants || {} };
    }).filter(Boolean);
...
// UPDATED: To render selected variants
function renderOrderItems() {
    const container = document.getElementById('order-items-container');
    container.innerHTML = orderItems.map(item => {
        const variantsHtml = item.selectedVariants && Object.keys(item.selectedVariants).length > 0
            ? `<div class="text-xs text-gray-500 mt-1">${Object.entries(item.selectedVariants).map(([key, value]) => `<span>${key}: ${value}</span>`).join(' &middot; ')}</div>`
            : '';
