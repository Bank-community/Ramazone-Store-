...
let ramazoneConfig = { deliveryCharge: 15, freeDeliveryThreshold: 500 };
...
function processCartForDisplay() {
    return getCart().map((cartItem, index) => {
...
// === UPDATE: New Price Summary Logic with MRP and Savings ===
function updatePriceSummary() {
    ...
    // Step 1 Summary
    let summaryStep1HTML = `<div class="flex justify-between"><span class="text-gray-600">Total MRP</span><span class="font-medium text-gray-800">₹${totalMRP.toLocaleString('en-IN')}</span></div>`;
    if (totalSavings > 0) {
        summaryStep1HTML += `<div class="flex justify-between text-green-600"><span class="font-semibold">Total Savings</span><span class="font-semibold">- ₹${totalSavings.toLocaleString('en-IN')}</span></div>`;
    }
    ...
    let summaryStep3HTML = `<div class="flex justify-between"><span class="text-gray-600">Subtotal</span><span class="font-medium">₹${subtotal.toLocaleString('en-IN')}</span></div>`;
    if (totalSavings > 0 || couponDiscount > 0) {
        const finalSavings = totalSavings + couponDiscount;
        summaryStep3HTML += `<div class="flex justify-between text-green-600"><span class="font-semibold">Total Savings</span><span class="font-semibold">- ₹${finalSavings.toLocaleString('en-IN')}</span></div>`;
    }
    summaryStep3HTML += `<div class="flex justify-between"><span class="text-gray-600">Delivery Fee</span><span class="font-medium">${deliveryFee > 0 ? `₹${deliveryFee.toLocaleString('en-IN')}` : 'Free'}</span></div>`;
...
function setupEventListeners() {
    ...
    // === UPDATE: Event listener for clickable stepper ===
...
function handleCartActions(event) {
    ...
    const itemInCart = cart.find((_, i) => i === cartIndex);
    if (!itemInCart) return;

    if (target.classList.contains('delete-item-btn')) { cart = cart.filter((_, i) => i !== cartIndex); }
    else if (target.classList.contains('qty-increase')) { itemInCart.quantity++; } 
    else if (target.classList.contains('qty-decrease')) { if (itemInCart.quantity > 1) itemInCart.quantity--; else cart = cart.filter((_, i) => i !== cartIndex); }
    saveCart(cart);
    loadOrderFromCart();
}
...
function updateProgressBar() {
    ...
    document.getElementById('stepper-progress').style.width = `${((currentStep - 1) / 2) * 100}%`;
}

// All other functions (Coupons, Order Placement, Order Status, Invoice) are retained and updated
