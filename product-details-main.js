...
function loadProduct(data) {
    ...
    if (data.images && Array.isArray(data.images)) {
        mediaItems.push(...data.images.map(src => ({ type: 'image', src: src })));
    }
    if (data.videoUrl) {
        mediaItems.push({ type: 'video', src: data.videoUrl, thumbnail: (data.images && data.images[0]) || 'https://i.ibb.co/My6h0gdd/20250706-230221.png' });
    }
    ...
    setupImageModal();
    setupShareButton();
    setupCouponButton(); // Sets up the main coupon button
    setupCouponModal();   // NEW: Sets up the new coupon modal's logic
...
function updatePriceDisplay() {
    ...
    document.getElementById('price-final').textContent = `₹${finalPrice.toLocaleString('en-IN')}`;
    ...
    if (appliedCoupon) {
        document.getElementById('price-coupon-discount').textContent = `- ₹${couponDiscount.toLocaleString('en-IN')}`;
        couponRow.style.display = 'flex';
    } else {
        couponRow.style.display = 'none';
    }
    document.getElementById('price-delivery-charge').textContent = deliveryCharge > 0 ? `+ ₹${deliveryCharge.toLocaleString('en-IN')}` : 'Free';
...
function getYoutubeEmbedUrl(url) {
    ...
    try {
        ...
        return videoId ? `https://www.youtube.com/embed/${videoId}?controls=1&rel=0&modestbranding=1` : null;
    } catch (e) {
...
// ... (Other functions like showMedia, setupSliderControls, etc. remain unchanged) ...
function showMedia(index){if(index<0||index>=mediaItems.length)return;slider.style.transition="transform 0.3s ease-out",currentMediaIndex=index,currentTranslate=index*-sliderWrapper.offsetWidth,prevTranslate=currentTranslate,setSliderPosition(),document.querySelectorAll(".thumbnail").forEach((thumb,i)=>thumb.classList.toggle("active",i===index))}function setupSliderControls(){sliderWrapper.addEventListener("touchstart",touchStart,{passive:!0}),sliderWrapper.addEventListener("touchend",touchEnd),sliderWrapper.addEventListener("touchmove",touchMove,{passive:!0}),sliderWrapper.addEventListener("mousedown",touchStart),sliderWrapper.addEventListener("mouseup",touchEnd),sliderWrapper.addEventListener("mouseleave",touchEnd),sliderWrapper.addEventListener("mousemove",touchMove)}function touchStart(event){startPos=getPositionX(event),isDragging=!0,animationID=requestAnimationFrame(animation),slider.style.transition="none"}function touchMove(event){if(isDragging){const currentPosition=getPositionX(event);currentTranslate=prevTranslate+currentPosition-startPos}}function touchEnd(event){isDragging&&(isDragging=!1,cancelAnimationFrame(animationID),currentTranslate-prevTranslate<-50&&currentMediaIndex<mediaItems.length-1&&currentMediaIndex++,currentTranslate-prevTranslate>50&&currentMediaIndex>0&&currentMediaIndex--,showMedia(currentMediaIndex))}function getPositionX(event){return event.type.includes("mouse")?event.pageX:event.touches[0].clientX}function animation(){setSliderPosition(),isDragging&&requestAnimationFrame(animation)}function setSliderPosition(){slider.style.transform=`translateX(${currentTranslate}px)`}function setupQuantityControls(){document.getElementById("increase-quantity").addEventListener("click",()=>{productQuantity++,updateQuantityAndWhatsAppLink()}),document.getElementById("decrease-quantity").addEventListener("click",()=>{productQuantity>1&&(productQuantity--,updateQuantityAndWhatsAppLink())}),updateQuantityAndWhatsAppLink()}
function updateQuantityAndWhatsAppLink(){const sellerPhoneNumber="917903698180";document.getElementById("quantity-display").textContent=productQuantity,document.getElementById("decrease-quantity").disabled=productQuantity<=1;const basePrice=Number(currentProductData.displayPrice),couponDiscount=appliedCoupon?Number(appliedCoupon.discount):0,deliveryCharge="Ramazone"===selectedDeliveryType?ramazoneDeliveryCharge:0,singleItemFinalPrice=basePrice-couponDiscount,totalOrderPrice=singleItemFinalPrice*productQuantity+deliveryCharge,productName=currentProductData.name.replace(/\*/g,"").trim();let message=`🛍️ *Ramazone Store Order* 🛍️\n\nHello! I'm interested in this product:\n\n*Product:* *${productName}*\n`;if(Object.keys(selectedVariants).length>0){message+="*Options:*\n";for(const type in selectedVariants)message+=` - ${type}: ${selectedVariants[type]}\n`}message+=`*Quantity:* ${productQuantity}\n-------------------------------------\n*Price per item:* ₹${basePrice.toLocaleString("en-IN")}\n`,appliedCoupon&&(message+=`*Coupon Discount:* - ₹${couponDiscount.toLocaleString("en-IN")}\n`),message+=`*Delivery (${selectedDeliveryType}):* + ₹${deliveryCharge.toLocaleString("en-IN")}\n-------------------------------------\n*Total Price:* *₹${totalOrderPrice.toLocaleString("en-IN")}*\n\n*Product ID:* ${currentProductId}\n*Product Link:* ${window.location.href}\n\nPlease let me know if it's available.`,document.getElementById("whatsapp-order-link").href=`https://wa.me/${sellerPhoneNumber}?text=${encodeURIComponent(message)}`}
// --- MODIFIED & NEW FUNCTIONS FOR COUPON MODAL ---

/**
 * UPDATED: Replaces prompt() with a function to open the new custom modal.
 */
function setupCouponButton() {
    document.getElementById('coupon-btn').addEventListener('click', openCouponModal);
}

/**
 * NEW: Opens the coupon modal.
 */
...
/**
 * NEW: Sets up all event listeners for the coupon modal.
 */
    ...
    applyBtn.addEventListener('click', () => {
        ...
        if (code) {
            ...
            if (coupon) {
                ...
                appliedCoupon = null; // Important to reset if coupon is invalid
