...
function loadProduct(data){
    mediaItems=[],data.images&&Array.isArray(data.images)&&mediaItems.push(...data.images.map(src=>({type:"image",src:src}))),data.videoUrl&&mediaItems.push({type:"video",src:data.videoUrl,thumbnail:data.images&&data.images[0]||"https://i.ibb.co/My6h0gdd/20250706-230221.png"}),document.title=`${data.name||"Product"} - Ramazone`,document.querySelector('meta[property="og:title"]').setAttribute("content",data.name),document.querySelector('meta[property="og:image"]').setAttribute("content",data.images&&data.images[0]||"https://i.ibb.co/My6h0gdd/20250706-230221.png"),document.getElementById("product-title").textContent=data.name,data.rating&&data.reviewCount&&(document.getElementById("rating-section").style.display="flex",renderStars(data.rating,document.getElementById("product-rating-stars")),document.getElementById("product-review-count").textContent=`(${data.reviewCount} ratings)`);const descriptionContainer=document.getElementById("product-description");if(descriptionContainer.innerHTML="",data.description&&Array.isArray(data.description)){document.getElementById("description-section").style.display="block",data.description.forEach(block=>{const blockDiv=document.createElement("div");blockDiv.className="desc-block";let blockHTML="";block.title&&(blockHTML+=`<h3 class="desc-block-title">${block.title}</h3>`),block.details&&(blockHTML+=`<p class="desc-block-details">${block.details}</p>`),block.highlights&&Array.isArray(block.highlights)&&block.highlights.length>0&&(blockHTML+=`<ul class="desc-block-highlights">${block.highlights.map(h=>`<li>${h}</li>`).join("")}</ul>`),blockDiv.innerHTML=blockHTML,descriptionContainer.appendChild(blockDiv)})}const gallery=document.getElementById("thumbnail-gallery");gallery.innerHTML="",slider.innerHTML="",mediaItems.forEach((item,index)=>{const slide=document.createElement("div");slide.className="media-item","image"===item.type?slide.innerHTML=`<img src="${item.src}" alt="Product image ${index+1}" draggable="false">`:getYoutubeEmbedUrl(item.src)&&(slide.innerHTML=`<iframe src="${getYoutubeEmbedUrl(item.src)}" class="w-full h-auto object-cover aspect-square" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>`),slider.appendChild(slide);const thumbWrapper=document.createElement("div");thumbWrapper.className="aspect-square thumbnail";const img=document.createElement("img");if(img.src="image"===item.type?item.src:item.thumbnail,thumbWrapper.appendChild(img),"video"===item.type){const playIcon=document.createElement("div");playIcon.className="play-icon",playIcon.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" width="50%" height="50%"><path d="M8 5v14l11-7z"/></svg>',thumbWrapper.appendChild(playIcon)}thumbWrapper.addEventListener("click",()=>showMedia(index)),gallery.appendChild(thumbWrapper)}),showMedia(0),renderVariantSelectors(data.variants),setupSliderControls(),setupQuantityControls(),setupImageModal(),setupShareButton(),setupCouponButton(),setupCouponModal(),setupDeliveryOptions(),setupVariantModal(),updatePriceDisplay(),loadSimilarProducts(data.category),loadOtherProducts(data.category)
}
...
function updatePriceDisplay(){const basePrice=Number(currentProductData.displayPrice),originalPrice=Number(currentProductData.originalPrice),couponDiscount=appliedCoupon?Number(appliedCoupon.discount):0,deliveryCharge="Ramazone"===selectedDeliveryType?ramazoneDeliveryCharge:0,finalPrice=basePrice-couponDiscount;document.getElementById("price-final").textContent=`₹${finalPrice.toLocaleString("en-IN")}`;const percentageDiscountEl=document.getElementById("price-percentage-discount"),originalPriceEl=document.getElementById("price-original"),lowestPriceTag=document.getElementById("lowest-price-tag-container");if(originalPrice>basePrice){const discount=Math.round(100*((originalPrice-basePrice)/originalPrice));percentageDiscountEl.innerHTML=`<i class="fas fa-arrow-down mr-1"></i>${discount}%`,originalPriceEl.textContent=`₹${originalPrice.toLocaleString("en-IN")}`,percentageDiscountEl.style.display="flex",originalPriceEl.style.display="inline",lowestPriceTag.style.display="block"}else percentageDiscountEl.style.display="none",originalPriceEl.style.display="none",lowestPriceTag.style.display="none";const couponRow=document.getElementById("price-coupon-row");appliedCoupon?(document.getElementById("price-coupon-discount").textContent=`- ₹${couponDiscount.toLocaleString("en-IN")}`,couponRow.style.display="flex"):couponRow.style.display="none",document.getElementById("price-delivery-charge").textContent=deliveryCharge>0?`+ ₹${deliveryCharge.toLocaleString("en-IN")}`:"Free",updateOrderLink()}
...
function updateOrderLink(){
    ...
    const orderLink = document.getElementById('order-now-link'); 
    if (orderLink) {
        const variantsQueryString = encodeURIComponent(JSON.stringify(selectedVariants));
        // *** CHANGE: Pass applied coupon code in the URL ***
        const couponQueryString = appliedCoupon ? `&coupon=${appliedCoupon.code}` : '';
        orderLink.href = `order.html?id=${currentProductId}&quantity=${productQuantity}&variants=${variantsQueryString}${couponQueryString}`;
    }
}

function setupCouponButton(){document.getElementById("coupon-btn").addEventListener("click",()=>{openCouponModal()})}
...
function openCouponModal() {
    ...
    setTimeout(() => {
        ...
        overlay.querySelector('#coupon-modal-content').classList.remove('scale-95', 'opacity-0');
    }, 10);
}
...
function closeCouponModal() {
    ...
    overlay.querySelector('#coupon-modal-content').classList.add('scale-95', 'opacity-0');
    setTimeout(() => {
...
function setupCouponModal() {
    ...
    if (!overlay || !closeBtn || !applyBtn || !input) return;
