...
async function fetchConfigurations() {
    try {
        ...
        if (!firebaseResponse.ok || !imgbbResponse.ok) {
            throw new Error('Server se configuration fetch nahi ho saki.');
        }
        ...
        if (!firebaseConfig.apiKey || !imgbbConfig.apiKey) {
             throw new Error('API keys server response mein nahi hain.');
        }
        ...
        document.body.innerHTML = `<div style="text-align:center; padding: 50px; font-family: sans-serif; color: #333;"><h1>Error</h1><p>Application configuration load nahi ho saki. Kripya administrator se sampark karein.</p><p style="color: #d9534f; font-weight: bold;">${error.message}</p></div>`;
        return false;
    }
}


// --- CORE FUNCTIONS (Unchanged, bas ab dynamic keys ka istemal karenge) ---
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    loadingOverlay.innerHTML = `
        <div style="position:fixed; inset:0; background: #f9fafb; z-index:9999; display:flex; flex-direction:column; justify-content:center; align-items:center; gap:1rem; font-family: Inter, sans-serif;">
            <div class="loader"></div>
            <p>Panel ko surakshit roop se shuru kiya ja raha hai...</p>
        </div>`;
