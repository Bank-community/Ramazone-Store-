...
const IMGBB_API_KEY = 'ded41444ac8a63ec72af276487ae6f3d';
...
function setupEventListeners() {
    // Ye event listener har link ke liye sirf ek baar attach hoga
    ...
    // === NAYA CODE: Home Button ke liye event listener ===
    const homeButton = document.getElementById('home-button');
    if (homeButton && !homeButton.eventAttached) {
        homeButton.eventAttached = true; // Dobara listener attach hone se rokne ke liye
        homeButton.addEventListener('click', () => {
            const dashboardLink = document.querySelector('.sidebar-link[data-page="sections/dashboard.html"]');
            if (dashboardLink) {
                // Sidebar ke dashboard link par click simulate karte hain
                // Isse page loading ka logic reuse ho jayega
                dashboardLink.click();
            }
        });
    }
...
function renderProfilePage() {
    ...
    const profilePageHtml = `
        <div class="profile-page-header">
            <div>
                ...
                <p class="text-sm text-gray-500">View your details and manage your password.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="profile-card">
                ...
                <div class="profile-card-body">
                    <div class="space-y-4">
                        <div class="detail-item"><i class="fas fa-user-circle"></i> <strong>Full Name:</strong> <span class="ml-auto text-gray-700">${window.currentVendorData.name}</span></div>
                        <div class="detail-item"><i class="fas fa-store"></i> <strong>Shop Name:</strong> <span class="ml-auto text-gray-700">${window.currentVendorData.shopName}</span></div>
                        <div class="detail-item"><i class="fas fa-envelope"></i> <strong>Login Email:</strong> <span class="ml-auto text-gray-700">${window.currentVendorData.email}</span></div>
                        <div class="detail-item"><i class="fab fa-whatsapp"></i> <strong>WhatsApp:</strong> <span class="ml-auto text-gray-700">${window.currentVendorData.whatsappNumber}</span></div>
                    </div>
                </div>
            </div>
            <div class="profile-card">
                ...
                <div class="profile-card-body">
                    <form id="profile-change-password-form" class="space-y-4">
                        <div><label for="profile-old-password" class="form-label">Old Password</label><input type="password" id="profile-old-password" class="form-control" required autocomplete="current-password"></div>
                        <div><label for="profile-new-password" class="form-label">New Password</label><input type="password" id="profile-new-password" class="form-control" required autocomplete="new-password"></div>
                        <div><label for="profile-confirm-password" class="form-label">Confirm New Password</label><input type="password" id="profile-confirm-password" class="form-control" required autocomplete="new-password"></div>
    ...
    document.getElementById('profile-change-password-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const oldPassword = document.getElementById('profile-old-password').value; const newPassword = document.getElementById('profile-new-password').value; const confirmPassword = document.getElementById('profile-confirm-password').value;
        if (!oldPassword || !newPassword || !confirmPassword) return window.showToast('Error', 'Please fill all password fields.', 'error');
        if (newPassword !== confirmPassword) return window.showToast('Error', 'New passwords do not match.', 'error');
        if (newPassword.length < 6) return window.showToast('Error', 'Password must be at least 6 characters.', 'error');
        const updateBtn = document.getElementById('update-password-btn'); updateBtn.disabled = true; updateBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Updating...';
        const user = auth.currentUser; const credential = firebase.auth.EmailAuthProvider.credential(user.email, oldPassword);
        try {
            await user.reauthenticateWithCredential(credential); await user.updatePassword(newPassword);
            window.showToast('Success!', 'Your password has been updated successfully.', 'success'); document.getElementById('profile-change-password-form').reset();
        } catch (error) {
            console.error("Password change error:", error);
            let msg = 'An error occurred. Please try again.'; if (error.code === 'auth/wrong-password') msg = 'The old password you entered is incorrect.'; else if (error.code === 'auth/weak-password') msg = 'The new password is too weak.'; window.showToast('Update Failed', msg, 'error');
        } finally { updateBtn.disabled = false; updateBtn.innerHTML = 'Update Password'; }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        app = firebase.initializeApp(firebaseConfig); db = firebase.database(); auth = firebase.auth();
